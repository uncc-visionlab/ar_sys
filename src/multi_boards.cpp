...
public:
    ...
    void readFromFile(std::string sfile) throw ( cv::Exception) {
        try {
            ...
            throw cv::Exception(81818, "ArSysMultiBoards::readFromFile", ex.what() + std::string(" file=)") + sfile, __FILE__, __LINE__);
        }
    }
    ...
    void image_callback(const sensor_msgs::ImageConstPtr& msg) {
        ...
        try {
            ...
            if (ids.size() > 0) {
                cv::aruco::estimatePoseSingleMarkers(corners, boards[0].marker_size, 
